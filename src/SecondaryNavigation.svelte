...
<div class="dr-secondary-navigation-container">
  ...
  {#if $state.activePrimary}
    <div class="dr-secondary-navigation-header">
      <div class="flex-item">
        <img
          src="{$config.mediaPath + rail[$state.activePrimary].icon}.svg"
          alt={rail[$state.activePrimary].label}
        />
      </div>
      <div class="flex-item"><h2>{rail[$state.activePrimary].label}</h2></div>
    </div>
    {#if $state.activePrimary == "stories"}
      <div class="dr-secondary-navigation-items">
        {#each rail[$state.activePrimary].content as story}
          <div
            class="dr-secondary-navigation-item {$state.activeObject == story ? "active" : ""}"
            on:click={() => setActiveStory(story)}
          >
            <img src="{$config.mediaPath + story.thumbnail}" alt={story.title} />
            <h2>{story.title}</h2>
          </div>
        {/each}
      </div>
    ...
    {:else}
      <div class="dr-secondary-navigation-artifacts-list">
        {#each rail[$state.activePrimary].content as artifact, index}
            <div
              class="dr-secondary-navigation-artifact {$state.activeObject === artifact ? "active" : ""}"
