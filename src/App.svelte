<script>
	...
	async function getRailContent() {
		...
		try {
			...
			railName = content.identifier.substring(4)
		} catch (err) {
			console.log(err)
		}
		if (content) {
	...
	async function getRailConfig() {
		var req, res, config
		try {
			req = await fetch("./mediapool/config.json")
	...
	function setRailStyles(configObject) {
		...
		bS.setProperty("--dr-gallery-color", configObject[gallery].color)
		bS.setProperty("--dr-gallery-color-active", configObject[gallery].activeColor)
		if (configObject[gallery].hasOwnProperty("dateRangeColor")) {
			bS.setProperty("--dr-gallery-color-dateRange", configObject[gallery].dateRangeColor)
		}
	}

	let promise = getRailContent()
	getRailConfig().then((result) => {
		setRailStyles(result)
	})
